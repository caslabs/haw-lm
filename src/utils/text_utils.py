...
Author: Joe Winkie
Modified by: Jerald Dancel

The original module provides helper functions and the `InternalState` class for working with
Hawaiian-English book data. It is modified to include additional functionality
such as loading book covers, normalizing text, and managing in-memory caches.
...
class InternalState:
    ...
    def list_article_ids(self, bookID):
        """
        Lists all article IDs (articleID) for pages in a specific book ID.
        """
        pages = self.load_pages(bookID)
        if not pages:
            return []

        # Extract articleIDs from the loaded metadata
        article_ids = [page.get('articleID') for page in pages]

        return article_ids

    def clean_html(slef, html_text):
        """
        Cleans HTML text by removing tags and returning plain text.
